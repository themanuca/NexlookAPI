    ...
    public class IAIServiceController : ControllerBase
    {
        ...
        public async Task<IActionResult> GerarDescricaoImagemComFoto([FromBody] GerarDescricaoImagemRequest prompt)
        {
            try
            {
                ...
                if (descricao == null)
                {
                    _logger.LogWarning("Nenhum look gerado para usuário {UserId}. Armário vazio ou imagens inválidas.", userId);
                    return BadRequest(new { Mensagem = "Não foi possível gerar o look. Verifique se há peças no armário." });
                }
